...
def keep_loading_more(DRIVER: webdriver):
	...
	try:
		...
		fail(f'Error: Could not find "Load More" button in {TIMEOUT} seconds\n{err}')
		exit(-1)
	...
	try:
		while load_more_button != None:
			...
			log(f'({count}) Clicking "Load More" button')
...
def parse_schedule_weeks(DRIVER: webdriver) -> list[dict]:
	...
	try:
		log('Waiting for parent element to become visible')
		parent_elem = wait.until(EC.visibility_of_element_located((By.XPATH, SCHEDULE_PARENT_DIV_XPATH)))
	except Exception as err:
		fail(f'Error: Could not locate parent element in {TIMEOUT} seconds\n{err}')
		exit(-1)
...
def convert_nba_date(date_str: str) -> str:
	...
	return parsed_date.isoformat() + "Z" # Return as string in ISO format